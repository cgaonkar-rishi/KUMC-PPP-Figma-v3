...
export function HeaderOption7({ currentPage, onNavigate, isSidebarCollapsed, onToggleSidebar, currentUserRole = 'Admin', onRoleChange }: HeaderProps) {
  ...
  return (
    <header className="bg-white border-b border-gray-200 flex items-center h-16">
      ...
      <div className={`h-16 bg-white flex items-center justify-between px-6 border-r border-gray-200 transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        {!isSidebarCollapsed && (
          <div className="flex-1 mr-2">
            <svg viewBox="0 0 200 40" className="w-full h-10" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="KUMC Medical Center Logo">
        ...
        {isSidebarCollapsed && (
          <div className="w-12 h-12 flex items-center justify-center">
            <svg viewBox="0 0 40 40" className="w-full h-full" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="KUMC Logo">
              <title>KUMC</title>
      ...
      <div className="flex-1 flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-3">
          {/* Navigation Pills - Prominent badges with text */}
          <div className="flex items-center gap-2">
            <button 
              onClick={() => onNavigate?.('dashboard')}
              aria-label="Go to dashboard"
              className="px-4 py-2.5 bg-gradient-to-r from-[#0051BA] to-[#003A70] hover:from-[#003A70] hover:to-[#0051BA] text-white rounded-full transition-all duration-200 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-[#0051BA] focus:ring-offset-2 flex items-center gap-2 font-medium text-sm shadow-md"
            >
          ...
          <div className="flex items-center gap-2 ml-2">
            <div className="relative" ref={notificationsRef}>
              <button 
                ...
                aria-haspopup="true"
                className="relative p-2.5 hover:bg-amber-100 text-gray-600 hover:text-amber-700 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500 bg-gray-100"
              >
              ...
              {showNotifications && (
                ...
                >
                  ...
                  <div className="relative bg-gradient-to-br from-gray-500 via-gray-600 to-slate-700 p-5">
                    {/* Decorative Pattern */}
                    ...
                    <div className="relative flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-200">{unreadCount} unread messages</p>
                        </div>
                      </div>
                      <button className="text-xs text-white hover:text-gray-100 font-medium bg-white/20 px-3 py-1.5 rounded-lg hover:bg-white/30 transition-colors">
                        Mark all read
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-3 text-center border-t border-gray-100 bg-gray-50">
                    <button className="text-sm text-ku-blue hover:text-ku-blue-dark font-medium">View all notifications</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative" ref={settingsRef}>
              <button
                ...
                aria-haspopup="true"
                className="p-2.5 hover:bg-slate-200 text-gray-600 hover:text-slate-800 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-slate-600 bg-gray-100"
              >
            ...
            <div className="relative" ref={profileRef}>
              <button 
                ...
                aria-haspopup="true"
                className="relative focus:outline-none focus:ring-2 focus:ring-[#0051BA] focus:ring-offset-2 rounded-full"
              >
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0051BA] via-[#003A70] to-[#003A70] flex items-center justify-center text-white font-semibold text-sm shadow-md hover:shadow-lg transition-all duration-200 ring-2 ring-white hover:ring-blue-200 cursor-pointer">
                  AU
                </div>
              ...
              <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap shadow-sm border ${getRoleBadgeColor(currentUserRole)}`}>
                {currentUserRole === 'Admin' && '👑 Admin'}
                {currentUserRole === 'Study Coordinator' && '📋 Coordinator'}
                {currentUserRole === 'Read Only' && '👁️ Read Only'}
              </div>
            ...
            <div className="relative" ref={roleSwitcherRef}>
              <button
                ...
                aria-haspopup="true"
                className="p-2.5 hover:bg-gray-100 text-gray-600 hover:text-gray-800 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gray-600 bg-gray-100"
              >
              ...
              {showRoleSwitcher && (
                <div 
                  role="menu"
                  aria-label="Role switcher menu"
                  className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden"
                >
                  {/* Header Section with Gradient */}
                  <div className="relative bg-gradient-to-br from-gray-600 via-gray-700 to-gray-800 p-5">
                    {/* Decorative Pattern */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-white opacity-5 rounded-full -mr-16 -mt-16"></div>
                    <div className="absolute bottom-0 left-0 w-24 h-24 bg-white opacity-5 rounded-full -ml-12 -mb-12"></div>
                    ...
                    <div className="relative flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-white font-semibold text-lg">Switch Role</p>
                        <p className="text-xs text-gray-300">Change your current role</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-2">
                    ...
                    >
                      <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center group-hover:bg-blue-600 transition-colors">
                        <Crown size={16} className="text-blue-600 group-hover:text-white transition-colors" />
                      </div>
                      <div className="flex-1">
                    ...
                    <button
                      onClick={() => {
                        onRoleChange?.('Study Coordinator');
                        setShowRoleSwitcher(false);
                      }}
                      role="menuitem"
                      className={`w-full px-4 py-2.5 text-left rounded-lg hover:bg-gradient-to-r hover:from-green-50 hover:to-emerald-50 flex items-center gap-3 text-gray-700 transition-all duration-200 group focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${
                        currentUserRole === 'Study Coordinator' ? 'bg-green-50' : ''
                      }`}
                    >
                      <div className="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center group-hover:bg-green-600 transition-colors">
                        <Clipboard size={16} className="text-green-600 group-hover:text-white transition-colors" />
                      </div>
                      <div className="flex-1">
                        <span className="font-medium text-sm">Study Coordinator</span>
                        <p className="text-xs text-gray-500">Manage assigned studies</p>
                      </div>
                    </button>
                    
                    <button
                      onClick={() => {
                        onRoleChange?.('Read Only');
                        setShowRoleSwitcher(false);
                      }}
                      role="menuitem"
                      className={`w-full px-4 py-2.5 text-left rounded-lg hover:bg-gradient-to-r hover:from-gray-50 hover:to-slate-50 flex items-center gap-3 text-gray-700 transition-all duration-200 group focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 ${
                        currentUserRole === 'Read Only' ? 'bg-gray-50' : ''
                      }`}
                    >
                      <div className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center group-hover:bg-gray-600 transition-colors">
                        <Eye size={16} className="text-gray-600 group-hover:text-white transition-colors" />
                      </div>
                      <div className="flex-1">
                        <span className="font-medium text-sm">Read Only</span>
                        <p className="text-xs text-gray-500">View data only</p>
                      </div>
                    </button>
                  </div>
                </div>
