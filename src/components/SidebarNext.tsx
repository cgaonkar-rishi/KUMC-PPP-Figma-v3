...
export function SidebarNext({ isCollapsed, onToggleCollapse }: SidebarProps) {
  const pathname = usePathname();
  const [reportsExpanded, setReportsExpanded] = useState(false);
  const [paymentsExpanded, setPaymentsExpanded] = useState(false);

  // Convert pathname to page name for active state
  const currentPage = pathname === '/' ? 'studies' : pathname.substring(1);
      ...
      <nav className="flex-1 p-4 overflow-y-auto">
        <ul className="space-y-2">
          {menuItems.map((item) => {
            ...
            const isActive = currentPage === item.id || currentPage.startsWith(item.id + '-') || currentPage.startsWith(item.id + '/');
